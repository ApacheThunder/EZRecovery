...
static const u32 currentFWCRC = 0xB23F6EAE;
...
bool Check_FW_update(u16 Current_FW_ver, u16 Built_in_ver) {
	vu16 busy;
	vu32 offset;
	
	u32 get_crc32 = crc32((unsigned char*)newomega_top_bin, newomega_top_bin_size);
		...
		switch (keysDown()) {
			...
			case KEY_A:
				...
				for(offset = 0x0000; offset < newomega_top_bin_size; offset += 256) {
					finalResult = (offset*100/newomega_top_bin_size+1);
					if (!UpdateProgressText) {
						textBuffer = " Progress: ";
						statData = (offset*100/newomega_top_bin_size+1);
						UpdateProgressText = true;
					}
					
					// iprintf(" %lu%%", (offset*100/newomega_top_bin_size+1));
									
					FAT_table_buffer[0] = (0x40000 + offset);
					
					tonccpy(&FAT_table_buffer[1], (newomega_top_bin + offset), 256);
